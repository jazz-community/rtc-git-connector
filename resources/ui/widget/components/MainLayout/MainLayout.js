define([
    ...
    return declare(
        "com.siemens.bt.jazz.workitemeditor.rtcGitConnector.ui.widget.mainLayout",
        [_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin],
        {
                ...
                var saveButtonClick = function (event) {
                    ...
                    if (self._anyItemsToLink()) {
                        ...
                        var saveTheLinks = function () {
                            ...
                            // Save the links
                            self.jazzRestService.addLinksToWorkItem(
                                ...
                                function () {
                                    ...
                                    self.gitRestService.addBackLinksToGitHost(addBackLinksToGitHostParams).then(
                                        function (result) {
                                            ...
                                            if (event.target.id === "rtcGitConnectorSaveAndCloseButton") {
                                                ...
                                                self.mainDataStore.selectedRepositorySettings.set(
                                                    "repository",
                                                    selectedRepository
                                                );
                                            }
                        ...
                        var newIssueIndex = self.mainDataStore.selectedRepositoryData.issuesToLink.findIndex(
                            function (issue) {
                                return issue.id < 0;
                            }
                        );
                        ...
                        if (newIssueIndex >= 0) {
                            ...
                            self.gitRestService
                                .createNewIssue(selectedRepository, gitHost, accessToken, self.mainDataStore.workItem)
                                .then(
                                    function (result) {
                                        ...
                                        self.jazzRestService.addCreatedGitIssueTagToWorkItem(
                                            self.mainDataStore.workItem
                                        );
                                    ...
                                    function (error) {
                                        console.log(
                                            "Sorry! We could not create a new issue. " +
                                                "We'll still try to create any other links. Error message: " +
                                                error
                                        );
                                        saveTheLinks();
                                    }
            ...
            getInitialData: function () {
                ...
                this.jazzRestService
                    .getAllRegisteredGitRepositoriesForProjectArea(this.mainDataStore.projectArea.id)
                    .then(function (registeredGitRepositories) {
                        ...
                        self.mainDataStore.registeredGitRepositories.push.apply(
                            self.mainDataStore.registeredGitRepositories,
                            registeredGitRepositories
                        );

                        // Show an element if no repositories where found
                        domStyle.set(
                            "noRegisteredGitRepositoriesContainer",
                            "display",
                            !registeredGitRepositories.length ? "block" : "none"
                        );
                    });
            ...
            watchDataStore: function () {
                ...
                this.mainDataStore.selectedRepositorySettings.watch("gitHost", function (name, oldValue, value) {
                    ...
                    domStyle.set(
                        "invalidGitRepositoryTypeContainer",
                        "display",
                        valueIsValid || value.name === "" ? "none" : "block"
                    );
            ...
            determineSelectedRepositoryGitHost: function () {
                ...
                if (
                    ...
                    this.gitRestService
                        .determineRepositoryGitHost(this.mainDataStore.selectedRepositorySettings.get("repository"))
            ...
            getAccessTokenForSelectedRepository: function () {
                ...
                this.jazzRestService.getAccessTokenByHost(repositoryUrl.host).then(
                ...
                if (this.mainDataStore.selectedRepositoryData.issuesToLink.length > 0) {
                    ...
                    this.jazzRestService.createNewWorkItems(
                        ...
                        function (workItem, gitIssue) {
                            ...
                            self.gitRestService
                                .addBackLinksToGitHost(addBackLinksToGitHostParams)
                                .then(function (result) {
                                    // Add a label to the git issue to indicate that it's been created as a work item in RTC
                                    self.gitRestService.addCreatedWorkItemLabelToIssue(
                                        selectedRepository,
                                        gitHost,
                                        accessToken,
                                        gitIssue
            ...
            _anyItemsToLink: function () {
                return (
                    this.mainDataStore.selectedRepositoryData.commitsToLink.length > 0 ||
            ...
            _allListsLoaded: function () {
                return (
                    this.mainDataStore.selectedRepositorySettings.get("issuesLoaded") &&
