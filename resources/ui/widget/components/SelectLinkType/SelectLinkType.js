define([
    ...
    return declare(
        "com.siemens.bt.jazz.workitemeditor.rtcGitConnector.ui.widget.selectLinkType",
        [_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin],
        {
                    ...
                    if (
                        ...
                        self.gitRestService
                            .getRecentIssues(
                                ...
                                gitHost,
                                accessToken,
                                self.jazzRestService.getIssueLinksFromWorkItem(self.mainDataStore.workItem)
                            )
                            .then(
                                function (issues) {
                    ...
                    if (
                        ...
                        self.gitRestService
                            .getRecentRequests(
                                ...
                                gitHost,
                                accessToken,
                                self.jazzRestService.getRequestLinksFromWorkItem(self.mainDataStore.workItem)
                            )
                            .then(
                                function (requests) {
                    ...
                    if (
                        ...
                        self.gitRestService
                            .getRecentCommits(
                                ...
                                gitHost,
                                accessToken,
                                self.jazzRestService.getGitCommitLinksFromWorkItem(self.mainDataStore.workItem)
                            )
                            .then(
                                function (commits) {
            ...
            setEventHandlers: function () {
                ...
                query(".rtcGitConnectorSelectLinkType").on(".linkTypeItem:click", function (event) {
                    self.mainDataStore.selectedRepositorySettings.set(
                        "linkType",
                        event.target.getAttribute("data-link-type")
                    );
                });
            },
            ...
            setRequestsText: function (gitHost) {
                ...
                query(".rtcGitConnectorSelectLinkType .linkTypeItem[data-link-type='REQUEST']")[0].innerHTML =
                    requestsText;
                query(
                    ".rtcGitConnectorViewAndSelectContainer[data-link-type='REQUEST'] .rtcGitConnectorLabelText"
                )[0].innerHTML = "Recent " + requestsText;
                query("#viewRequestsToLinkContainer .rtcGitConnectorLabelText")[0].innerHTML =
                    requestsText + " to link";
            },
            ...
            showLoadingDataError: function (message) {
                var errorStart =
                    "There was a problem and the data could not be loaded.<br/>" +
            ...
            showViewAndSelectWidget: function (linkType) {
                query(".rtcGitConnectorViewAndSelectContainer").forEach(function (node) {
                    var displayValue = node.getAttribute("data-link-type") === linkType ? "block" : "none";
